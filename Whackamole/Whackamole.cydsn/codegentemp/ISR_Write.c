...
#if !defined(ISR_Write__REMOVED) /* Check for removal by optimization */
...
void ISR_Write_Start(void)
{
    ...
    /* Set the priority. */
    ISR_Write_SetPriority((uint8)ISR_Write_INTC_PRIOR_NUMBER);
...
{
    #ifdef ISR_Write_INTERRUPT_INTERRUPT_CALLBACK
        ISR_Write_Interrupt_InterruptCallback();
    #endif /* ISR_Write_INTERRUPT_INTERRUPT_CALLBACK */ 
...
*   before the component has been started use ISR_Write_StartEx instead.
* 
*   When defining ISR functions, the CY_ISR and CY_ISR_PROTO macros should be 
*   used to provide consistent definition across compilers:
*
...
{
    CyRamVectors[CYINT_IRQ_BASE + ISR_Write__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + ISR_Write__INTC_NUMBER];
}
...
*
*   Note calling ISR_Write_Start or ISR_Write_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after ISR_Write_Start or ISR_Write_StartEx has been called. 
...
void ISR_Write_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((ISR_Write__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *ISR_Write_INTC_PRIOR = (*ISR_Write_INTC_PRIOR & (uint32)(~ISR_Write__INTC_PRIOR_MASK)) |
...
uint8 ISR_Write_GetPriority(void)
{
    ...
    priority = (*ISR_Write_INTC_PRIOR & ISR_Write__INTC_PRIOR_MASK) >> priorityOffset;
...
uint8 ISR_Write_GetState(void)
{
    ...
    return ((*ISR_Write_INTC_SET_EN & (uint32)ISR_Write__INTC_MASK) != 0u) ? 1u:0u;
}
